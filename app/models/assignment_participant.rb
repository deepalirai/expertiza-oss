...
# END of contributor methods
  ...
  def get_wiki_submissions
    ...
    if self.assignment.team_assignment
      wiki = Wikitype.find_by_name(self.assignment.wiki_type.name)
      submissions = Array.new
      if self.team
        self.team.get_participants.each {
            | user |
          val = wiki.review_mediawiki_group(self.assignment.directory_path, currenttime, user.handle)
          if val != nil
            submissions << val
          end
        }
      ...
      wiki = Wikitype.find_by_name(self.assignment.wiki_type.name)
      wiki.review_wiki(self.assignment.directory_path, currenttime, self.handle)
=begin
      return WikiType.review_wiki(self.assignment.directory_path, currenttime, self.handle)
      ...
      return WikiType.review_docuwiki(self.assignment.directory_path, currenttime, self.handle, n)
    else
      return Array.new
=end
    end
